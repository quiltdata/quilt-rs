...
impl<'a> Domain<'a> {
  ...
  async fn get_latest(&self, name: &str) -> Option<Manifest4<'a>> {
  ...
  async fn get_manifest<'a1>(&self, hash: &str) -> Option<Manifest4<'a>> {
  ...
  async fn insert_manifest(&mut self, manifest: &Manifest4<'a>) {
