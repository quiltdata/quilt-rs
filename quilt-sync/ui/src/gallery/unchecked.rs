    ...
    view! {
        <Scene
            ...
            note="One cause, host-grouped, in the region that already exists to name a \
                  ...
                  signed-out row under it: same grammar, same slot. Expand it and the \
                  packages are named, so the sentence and the list it speaks for cannot \
                  disagree. `[Try again]` re-checks only these packages, which the appbar's \
                  Refresh cannot do. Below the region the rows stay dashed and dimmed — \
                  switch to dark and watch how much the dim alone carries: 1.73:1 against a \
                  settled row."
        >
...
fn AllFailed() -> impl IntoView {
    ...
    view! {
        <Scene
            ...
            note="The likely case: no network, every call rejects, and not one row is \
                  confirmed. Without a cause row the region is not merely quiet but absent — \
                  no card, no heading, no zero line — so the reader is left with a page of \
                  grey rows and no statement. That is what settled this between the two \
                  candidate homes. Here one sentence accounts for the whole page, standing \
                  where `Everything is Latest` stands on a good day."
        >
