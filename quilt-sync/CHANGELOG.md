...
- **New main page** is now **New design preview** (Settings → Experimental, off by default) — one switch for the redesigned QuiltSync rather than one per page, so each part of the new design arrives under the switch you already turned on. If you had **New main page** on, nothing changes and nothing is lost. **New package page** is gone from Settings: the rebuilt package screen is still being built, and is now reachable only in a development build (<https://github.com/quiltdata/quilt-rs/pull/980>)
