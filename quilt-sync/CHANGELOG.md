...
- **New package page** now shows which revision this computer has, when it obtained it, and the package's S3 bucket in a read-only context pane. A package that lives only on this computer says it has no S3 bucket.
