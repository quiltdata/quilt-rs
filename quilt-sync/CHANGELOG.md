...
- Autopush-paused banners now append an explicit recovery hint — the watcher's `paused` set is only cleared on a manual Commit & Push (or Pull / Push / Set Remote), so fixing the underlying issue (e.g., correcting a typo'd workflow in publish settings) doesn't on its own make the banner go away. The new hint also warns that the cached error string may be stale relative to the current settings (<https://github.com/quiltdata/quilt-rs/pull/682>)
- Detail page's autosync paused banner now renders only for `Other(_)` pauses; for `Diverged` / `PendingChanges` / `PendingCommit`, the existing status banner already covers the state. Previously the snapshot hydration on page mount could stack two banners conveying the same information (<https://github.com/quiltdata/quilt-rs/pull/682>)
