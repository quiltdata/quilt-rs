...
- Every link to a package whose name contains `&`, `#` or `=` now carries the whole name. The name went into the address unescaped, so it ended at that character — `team/a&b` arrived as `team/a` — on the links to a package's own page, its commit page and its merge page, from both the current main page and the new one (<https://github.com/quiltdata/quilt-rs/pull/945>)
