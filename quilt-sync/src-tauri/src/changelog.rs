...
pub struct ChangelogEntry {
    /// Version string, e.g. "v0.22.4" or "v0.22.4-dev".
    pub version: String,
    /// Release date, e.g. "2026-04-02"; empty for an unreleased `-dev` section.
    pub date: String,
...
pub fn latest_entries() -> Vec<ChangelogEntry> {
    ...
    // Split on version headers: ## [vX.Y.Z] - YYYY-MM-DD, or ## [vX.Y.Z-dev]
    for chunk in CHANGELOG.split("\n## [") {
        ...
        // The preamble before the first header has no `]` on its first line.
        let Some((header, body)) = chunk.split_once('\n') else {
            continue;
        };

        // Parse "vX.Y.Z] - 2026-04-02" or "vX.Y.Z-dev]"
...
mod tests {
    ...
    /// The popup opens on the running build's own notes, released or `-dev`.
    #[test]
    fn test_first_entry_is_this_build() {
        let entries = latest_entries();
        assert_eq!(
            entries[0].version,
