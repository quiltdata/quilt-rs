...
const WORKFLOWS_CONFIG_KEY: &str = ".quilt/workflows/config.yml";
...
fn workflows_config_to_commit_workflows(
    ...
    match config {
        Ok(Some(config)) => {
            let config_url = catalog_object_url(
                &S3Uri {
            ...
            let workflows = config
                ...
                .iter()
                .map(|w| {
                    // A misconfigured `schemas` section degrades to no links
                    // rather than dropping the whole dialog.
                    let schemas = config.schema_uris(&w.id).unwrap_or_default();
                    CommitWorkflowInfo {
                        id: w.id.clone(),
                        name: w.name.clone(),
                        description: w.description.clone(),
                        metadata_schema_url: schemas
                            .metadata_schema
                            .and_then(|uri| catalog_object_url(&uri, host)),
                        entries_schema_url: schemas
                            .entries_schema
                            .and_then(|uri| catalog_object_url(&uri, host)),
                    }
                })
...
async fn get_commit_data_from_model(
    ...
    let workflows = workflows_config_to_commit_workflows(
        ...
        origin_host,
        typed_uri.as_ref().map_or("", |u| u.bucket.as_str()),
    );
...
async fn get_bucket_workflows_from_model(
    ...
    workflows_config_to_commit_workflows(config, host.as_ref(), bucket)
}
