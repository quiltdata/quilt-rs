...
def download_asset(asset, dest_dir, token):
    name = asset["name"]
    # Use API URL with octet-stream header for reliable downloads from private repos
...
def update_latest_json_urls(latest_json, upload_map, hubfs_root_url, base_path):
    ...
    def replace_url(value):
        ...
        if not parsed.scheme:
            ...
            # Split protocol to safely normalize path
            if "://" in full_path:
                proto, rest = full_path.split("://", 1)
                return f"{proto}://{rest.replace('//', '/')}"
            return full_path.replace("//", "/")
        
        # Handle basename case similarly
        base = f"{hubfs_root_url}{base_path}"
        full_path = f"{base}/{basename}"
        if "://" in full_path:
            proto, rest = full_path.split("://", 1)
            return f"{proto}://{rest.replace('//', '/')}"
        return full_path.replace("//", "/")
...
def main():
    ...
    # Ensure hubfs_root_url is absolute (has scheme)
    if not args.hubfs_root_url.startswith("http"):
        args.hubfs_root_url = "https://" + args.hubfs_root_url.lstrip("/")
