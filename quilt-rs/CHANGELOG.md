...
- `flow::install_paths` and `InstalledPackage::install_paths` refuse bytes from the remote that do not hash to the row being installed, with the new `InstallPathError::ContentMismatch` naming the path, and place nothing. A row whose `physical_key` has no `versionId` names the object as it is now, so after a later revision replaced it, downloading an older revision filed the later revision's bytes under the older row. The untouched file then read as modified, which makes a pull refuse, and a retry took the same bytes from the object store as a cache hit. Fetched bytes are now checked before they enter the store (<https://github.com/quiltdata/quilt-rs/pull/991>)
