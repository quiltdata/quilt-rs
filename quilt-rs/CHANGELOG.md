...
- `InstalledPackage::resolve_comparison(&lineage)` and `flow::compare_for_resolve` compare this copy's revision with the published `latest`, as the choice between them needs. They give the published revision's message; the files that differ, row by row on content hash; and how many pending revisions the registry does not list. `latest` is read fresh rather than taken from the lineage. With no catalog host nothing is asked, and every pending revision counts (<https://github.com/quiltdata/quilt-rs/pull/994>)
...
- Unversioned buckets: `flow::install_paths` and `InstalledPackage::install_paths` check fetched bytes against the row's hash before they enter the object store. A `physical_key` with no `versionId` names the object as it is now, so once a later revision replaced it, downloading an older revision filed the later revision's bytes under the older row. The untouched file then read as modified, and a retry took the same bytes from the store as a cache hit. Now a row that does not verify is skipped: every other row is installed, and the call returns the skipped paths. `InstalledPackage::install_paths` returns the new `flow::InstallPathsReport` (`paths`, as before, plus `skipped`), and `flow::install_paths` returns `(PackageLineage, Vec<PathBuf>)`. The pull's reconcile, which cannot apply part of a revision, refuses instead with the new `InstallPathError::ContentMismatch`. Packages on versioned buckets pin every key, so they are not affected. Found while diagnosing `qhq-a4za`, but it is not that bug's cause (<https://github.com/quiltdata/quilt-rs/pull/991>)
...
- `InstalledPackageStatus::most_recent_mtime` (newest `mtime` across non-ignored files) and a `working_tree_quiet(now, quiet_window)` helper for autosync's quiet-window guard (<https://github.com/quiltdata/quilt-rs/pull/682>)

### Changed

- `InstalledPackage::status` no longer persists lineage; the `latest_hash` refresh moved into `InstalledPackage::pull`, which now calls `flow::refresh_latest_hash` before the up-to-date guard (<https://github.com/quiltdata/quilt-rs/pull/682>)
...
- `flow::reset_to_latest` now clears `lineage.commit` so a subsequent certify cannot resurrect the discarded revision (<https://github.com/quiltdata/quilt-rs/pull/677>)
- `UpstreamState::from(PackageLineage)` now returns `Diverged` (was `Local`) when `remote.hash` is empty but `latest_hash` is not — flags namespaces another client has already published under, so autosync refuses to clobber them (<https://github.com/quiltdata/quilt-rs/pull/682>)

### Fixed

- `flow::pull_package` now reloads the manifest after install; previously `install_paths` read row hashes from the OLD revision and left stale bytes on disk after a "successful" pull (<https://github.com/quiltdata/quilt-rs/pull/682>)
