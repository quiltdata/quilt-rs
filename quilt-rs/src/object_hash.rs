//! Type-safe object content hashes.
//!
//! This module owns the per-object hashing concept: the algorithm-specific
//! hash types (`Sha256Hash`, `Sha256ChunkedHash`, `Crc64Hash`), the umbrella
//! [`ObjectHash`], and their multihash/multibase conversions. It has no
//! knowledge of manifests, storage, or S3 — the domain wrappers that compute
//! hashes for manifest rows live in `crate::checksum`.
...
impl TryFrom<Multihash<256>> for ObjectHash {
    ...
    fn try_from(multihash: Multihash<256>) -> Result<Self, Self::Error> {
        match multihash.code() {
            ...
            _ => Err(Error::InvalidMultihash(format!(
...
mod tests {
    ...
    fn test_object_hash_json_format_translation() -> Res {
        ...
        match object_hash {
            ...
            _ => {
                return Err(crate::Error::ObjectHash(Error::InvalidMultihash(
